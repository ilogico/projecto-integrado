...
class DeflateDecoder
	...
	class CompressedParser
		...
		copy: ->
			...
			# console.log distance, length, @decoder.window.length
			for i in [0...length]
				byte = @decoder.window[i + idx]
		...
		parse: (bit)->
			...
			if @bits == 0
				...
				if blockType == 0
					...
					@decoder.parser = new CompressedParser(@decoder, HuffmanTree.defaultLitTree, defaultDistTree)
				else if blockType == 2
...
test = ->
	require('fs').readFile "./crime.txt.deflate", (e, d)->
		throw e if e
		dec = new DeflateDecoder()
		dec.writeBuffer(d)
		unz = dec.getByteArray()
		console.log d.length, unz.length
		b = new Buffer(unz.length)
		for byte, i in unz
			b[i] = byte
		require('fs').writeFile "./crime.txt", b, (e)->
			throw e if e
